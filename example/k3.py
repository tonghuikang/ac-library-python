...
class Fancy:
    ...
    def __init__(self):
        ...
        self.e = (0), 0
        self.id_ = (1), (0)
        self.a = [((0), 1) for ai in range(10**5)]
        self.lazy_segtree = LazySegTree(self.op, self.e, self.mapping, self.composition, self.id_, self.a)
        self.cur = 0
        return

    def op(self, x: (int, int), y: (int, int)) -> (int, int):
        # with ModContext(self.mod):
            return x[0] + y[0], x[1] + y[1]

    def mapping(self, x: (int, int), y: (int, int)) -> (int, int):
        # with ModContext(self.mod):
            return x[0] * y[0] + x[1] * y[1], y[1]

    def composition(self, x: (int, int),
                    y: (int, int)) -> (int, int):
        # with ModContext(self.mod):
    ...
    def append(self, val: int) -> None:
        # with ModContext(self.mod):
            self.lazy_segtree.apply(self.cur, self.cur+1, ((0), (val)))
            self.cur += 1
            return
    ...
    def addAll(self, inc: int) -> None:
        # with ModContext(self.mod):
            self.lazy_segtree.apply(0, self.cur+1, ((1), (inc)))
            return

    def multAll(self, m: int) -> None:
        # with ModContext(self.mod):
            self.lazy_segtree.apply(0, self.cur+1, ((m), (0)))
            return
    ...
    def getIndex(self, idx: int) -> int:
        with ModContext(self.mod):
            if idx >= self.cur:
                return -1
            return self.lazy_segtree.get(idx)[0]
...
#     def composition(self, x: (int, int), y: (int, int)) -> (int, int):
#         return (x[0] * y[0]) % mod, (x[0] * y[1] + x[1]) % mod

#     id_ = 1, 0

#     self.lazy_segtree = LazySegTree(op, self.e, mapping, composition, id_, a)
