...
class Fancy:
    ...
    def __init__(self):
        self.mod = 10**9+7
        with ModContext(self.mod):
            self.e = Modint(0), 0
            self.id_ = Modint(1), Modint(0)
            self.a = [(Modint(0), 1) for ai in range(10**5)]
            self.lazy_segtree = LazySegTree(self.op, self.e, self.mapping, self.composition, self.id_, self.a)
            self.cur = 0
        return

    def op(self, x: (Modint, int), y: (Modint, int)) -> (Modint, int):
        with ModContext(self.mod):
            return x[0] + y[0], x[1] + y[1]

    def mapping(self, x: (Modint, Modint), y: (Modint, int)) -> (Modint, int):
        with ModContext(self.mod):
            return x[0] * y[0] + x[1] * y[1], y[1]

    def composition(self, x: (Modint, Modint),
    ...
    def append(self, val: int) -> None:
        with ModContext(self.mod):
            self.lazy_segtree.apply(self.cur, self.cur+1, (Modint(0), Modint(val)))
            self.cur += 1
            return
    ...
    def addAll(self, inc: int) -> None:
        with ModContext(self.mod):
            self.lazy_segtree.apply(0, self.cur+1, (Modint(1), Modint(inc)))
            return

    def multAll(self, m: int) -> None:
        with ModContext(self.mod):
            self.lazy_segtree.apply(0, self.cur+1, (Modint(m), Modint(0)))
            return
    ...
    def getIndex(self, idx: int) -> int:
        with ModContext(self.mod):
            if idx >= self.cur:
                return -1
            return self.lazy_segtree.get(idx)[0].val()


#     with ModContext(10**9+7):
#         # n, q = map(int, sys.stdin.readline().split())
#         self.a = [(Modint(0), 1) for _ range(10**5+1)]
