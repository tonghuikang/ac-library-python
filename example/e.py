...
def solve(grid, s):  # fix inputs here
    console("----- solving ------")
    console(grid, s)
    # grid = [(x-1, y-1, z) for x,y,z in grid]
    ...
    with ModContext(998244353):
        ...
        for i in range(s):
            arr.append(cur)
            cur = (cur*10) % 998244353
        arr = arr[::-1]
        console(arr)
        a = [(Modint(ai), 1) for ai in arr]
...
for case_num in [1]:
    ...
    for nrow in range(nrows):
        grid.append(list(map(int,inp[nrow + 1].split())))

    res = solve(grid, s)  # please change
